# This file is generated FFI::FileProcessor from lib/zlib.rb.ffi.
...
module Zlib
  ...
  class ZStream < FFI::Struct
    ...
    #
    # Wrapper for layout generation for subclasses
    # since the inheritance is otherwise troublesome.
    #
    ...
    def expand_output
      if @output.nil? then
        @output = ''
        @next_out = FFI::MemoryPointer.new CHUNK if @next_out.nil?
      else
    ...
    def run(data, flush)
      if @input.nil? and data.empty? then
        ...
        data_in = FFI::MemoryPointer.new @input.length
  ...
  attach_function :deflateSetDictionary, [:pointer, :string, :uint], :int
  ...
  attach_function :inflateSetDictionary, [:pointer, :string, :uint], :int

  attach_function :adler32_c,       "adler32",  [:ulong, :string, :uint], :ulong
  attach_function :crc32_c,         "crc32",    [:ulong, :string, :uint], :ulong
  ...
  class Deflate < ZStream
    ...
    # Explicitly set the layout to avoid inheritance problems.
    use_zstream_layout
  ...
  class GzipReader < GzipFile # HACK use a buffer class
    ...
    def self.open(*args)
      ...
      args[0] = File.new StringValue(args[0]), "rb"
    ...
    def eof?
      @zstream.finished? and @zstream.input_empty? and @buffer.empty?
    end
  ...
  class GzipWriter < GzipFile # HACK use a buffer class
    ...
    def self.open(*args)
      ...
      args[0] = File.new StringValue(args[0]), "wb"
  ...
  class Inflate < ZStream
    ...
    def inflate(data)
      data = Rubinius::Type.coerce_to data, String, :to_str unless data.nil?
